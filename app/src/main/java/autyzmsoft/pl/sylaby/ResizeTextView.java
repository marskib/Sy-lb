...
class ViewUtil {
    ...
    public static void resizeText(TextView textView, int originalTextSize, int minTextSize) {
        ...
        float ratioH = width / paint.measureText(textView.getText().toString());
        float ratioV = height / paint.measureText(textView.getText().toString());

        float rozmiarSkibH = 0.8f*originalTextSize * ratioH;
        float rozmiarSkibV = 0.8f*originalTextSize * ratioV;

        if (ratioH <= 1.0f) {
            textView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    Math.max(minTextSize,  rozmiarSkibH   /* originalTextSize * ratio */));
        };

        if (ratioV <= 1.2f) {
            textView.setTextSize(TypedValue.COMPLEX_UNIT_PX,
                    Math.max(minTextSize,  rozmiarSkibV   /* originalTextSize * ratio */));
        }
